...
const pokedex = function(){
    ...
    const draw_poke_list = function(new_list){
        ...
        new_list.forEach(item => {
            ...
            let height_ = 150;
            if(item.height > 12){
                height_ = 150 - (150 * ((item.height-16) / item.height));
            }
            const card = { 
                innerHTML:`
            ...
            fs.className = "col-5 bg-target bg-target-" + item.primary_type + " rounded-4 p-3 ms-1 me-1 pb-1 mb-2 pt-2";
...
setTimeout(() => {pok.dibujar_pokedex();},1000);
